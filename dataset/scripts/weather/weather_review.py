...
def get_weather_data_for_station(station_id):
    ...
    #os.makedirsrs(output_folder, exist_ok=True)
...
def start_remove_columns(station_ids):
    max_workers = min(os.cpu_count(), len(station_ids))  #Maximal 10 Threads oder so viele Stationen wie vorhanden
    print(f"Starte die Löschen von Spalten für {len(station_ids)} Stationen mit {max_workers} Threads.")
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        #Jede Station wird parallel heruntergeladen
        future_to_station = {executor.submit(remove_columns, station_id): station_id for station_id in station_ids}        
        for future in concurrent.futures.as_completed(future_to_station):
            station_id = future_to_station[future]
            try:
                future.result()  #Funktion ausführen und Fehler abfangen
...
def combine_historic_recent(station_id):
    ...
    #Aufruf nur benutzen, wenn start_... in weather nicht ausgeführt wird
    combine_all_station_data(station_id)
...
def combine_all_station_data(station_id):
    computing_folder_station = os.path.join(computing_folder, station_id)
    output_folder_station = os.path.join(output_folder, station_id) 
    #os.makedirsrs(output_folder_station, exist_ok=True)  
    ...
    if all_data_frames:
        ...
        #Speichern in Datei
        final_filename = os.path.join(output_folder_station, f"{station_id}_data_combined.csv")
...
def start_combine_all_station_data(station_ids):
    max_workers = min(os.cpu_count(), len(station_ids))  #Maximal 10 Threads oder so viele Stationen wie vorhanden
    print(f"Starte die Verknüfung aller Daten für {len(station_ids)} Stationen mit {max_workers} Threads.")
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        #Jede Station wird parallel heruntergeladen
        future_to_station = {executor.submit(combine_all_station_data, station_id): station_id for station_id in station_ids}        
        for future in concurrent.futures.as_completed(future_to_station):
            station_id = future_to_station[future]
            try:
                future.result()  #Funktion ausführen und Fehler abfangen
