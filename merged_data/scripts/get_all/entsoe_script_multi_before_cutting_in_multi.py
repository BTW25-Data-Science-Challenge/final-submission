...
def start_fetch_save_data(tasks):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_task = {
            executor.submit(fetch_save_data, task['query_func'], task['save_path'],
                            task.get('rename_columns'), task.get('transform_func')): task
            for task in tasks
        }
...
print(f'Ausführungszeit nach fetch_save: {verstrichene_zeit} Sekunden')
...
end_time = time.time()
verstrichene_zeit = end_time - start_time
print(f'Ausführungszeit nach Merge vor save als csv: {verstrichene_zeit} Sekunden')

merged_df.to_csv(f'{out_dir}/merged_data_multi.csv', index=False)

end_time = time.time()
verstrichene_zeit = end_time - start_time
print(f'Ausführungszeit nach Merge_to_csv: {verstrichene_zeit} Sekunden')

# Final filtering and saving
df = pd.read_csv(f'{out_dir}/merged_data_multi.csv')
df['Date'] = pd.to_datetime(df['Date'])
df_filtered = df[df['Date'].dt.minute == 0]
df_filtered['Date'] = df_filtered['Date'].dt.strftime('%Y-%m-%d %H:%M:%S')
df_filtered.to_csv(f'{out_dir}/merged_data_multi_filtered.csv', index=False)
